  ...
  try {
    ...
    while (!videoListo && intentos < POLL_MAX_TRIES) {
      ...
      if (estado.ready && estado.url) {
        ...
        await logAndNotify(
          sessionId,
          `Video still processing... intento ${intentos + 1}/${POLL_MAX_TRIES}`
        );
    ...
    if (!videoListo) {
      await logAndNotify(
        sessionId,
        "Normal verification timed out, attempting alternative methods..."
      );

      // EMPUJÓN 1: Intentar obtener el video una vez más con más tiempo
      try {
        await logAndNotify(sessionId, "Additional verification attempt...");
        ...
        if (estadoFinal.url) {
          ...
          videoListo = true;
            await logAndNotify(sessionId, "Video found on additional attempt");
        }
      } catch (extraCheckError) {
        await logAndNotify(sessionId, `Additional verification attempt failed: ${extraCheckError.message}`);
      }
      ...
      if (!videoListo) {
        await logAndNotify(
          sessionId,
          `Video not completed after ${intentos} attempts (~${Math.round((FIRST_WAIT_MS + (POLL_MAX_TRIES * POLL_INTERVAL_MS)) / 60000)} minutes)`
        );
        await logAndNotify(
          sessionId,
          `Generation ID for manual rescue: ${videoGeneration.generationId}`
        );
        await logAndNotify(
          sessionId,
          `Check your Hedra dashboard: https://app.hedra.com/`
        );
        await logAndNotify(
          sessionId,
          `The video may complete in the next few minutes`
        );
        await logAndNotify(
          sessionId,
          `You can try generating another video in the meantime`
        );
        
        throw new Error(
          `Video not completed after ${intentos} attempts (${Math.round((FIRST_WAIT_MS + (POLL_MAX_TRIES * POLL_INTERVAL_MS)) / 60000)} min). Generation ID: ${videoGeneration.generationId}`
        );
      }
    }
